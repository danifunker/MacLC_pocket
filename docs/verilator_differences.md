...
| Fetch-cache `.enable` (2026-08-22) | whatever a revived `sim.v` ties it to — **`1'b1` is correct in sim** (there is no fitter to fold anything) | **MUST be a non-constant net** evaluating to 1: `mac_lc_pocket.icache_en` ← `~icoff_s2` ← core_top's `opt_icache_off` reg at bridge `0xF0000064` (defaults 0 = cache on) | ★★★ The one place where sim and FPGA must deliberately DIFFER. Hardwiring `1'b1` on FPGA lets Quartus fold `fetch_cache`'s `enable_r` out of the `hit`/`hit_now` cones and the result crashes the Finder at 8bpp + 32-bit addressing (MiSTer **and** Pocket — shared-RTL, not fit lottery). Sim cannot observe this class at all: it is a synthesis-structure defect, so **a passing boot oracle is not evidence here**. Mechanism + repro: `docs/RESUME.md` §-7 |
