...
module tb_ism_gaptest;
	...
	begin
		...
		while (guard < 400000 && pops < POPS) begin
			guard = guard + 1;
			if (pops == 300 && !gap_done) begin
				gap_done = 1;
				$display("GAPTEST: injecting 60us poll stall at pop 300");
				repeat (1920) @(posedge clk);
			end
			swim_rd(4'h7, hs);
			if (hs[7]) begin
				swim_rd(4'h1, v);
		...
		swim_rd(4'h2, v);
		$display("GAPTEST: error register after field = %02x (b0=read-overrun b2=underrun)", v);
		...
		begin : parse_track
				...
				if (popbuf[k-3]==8'hA1 && popbuf[k-2]==8'hA1 && popbuf[k-1]==8'hA1 && popbuf[k]==8'hFE) begin
					...
					if (j < pops - 3 && popbuf[j]==8'hFB && (pops - (j+1)) >= 512) begin
						if (c != exp_track || h != exp_side) begin
							$display("GAPTEST: trk%0d side%0d — CHRN C=%0d H=%0d R=%0d (SEEK/SIDE WRONG)", exp_track, exp_side, c, h, r);
							...
							if ((j + 514) < pops && hsbuf[j+514][1] !== 1'b0) begin
								$display("GAPTEST: trk%0d side%0d sec%0d — DATA OK but CRC FLAG BAD (hs=%02x) ** DRIVER-VISIBLE FAIL **",
								         exp_track, exp_side, r, hsbuf[j+514]);
								total_fail = total_fail + 1;
							end
							if (errs) begin
								total_fail = total_fail + 1;
								$display("GAPTEST: trk%0d side%0d sec%0d — %0d/512 mismatches ** FAIL **", exp_track, exp_side, r, errs);
							end
							found = found + 1;
						end
					end
			...
			$display("GAPTEST: trk%0d side%0d — %0d/18 sectors captured%s", exp_track, exp_side, found,
			         (found >= 18) ? "" : "  ** INCOMPLETE **");
	...
	initial begin
		...
		for (ti = 22; ti <= 22; ti = ti + 1) begin
			seek_to(ti);
			for (si = 0; si < 2; si = si + 1)
				read_and_verify(ti, si);
		end

		$display("GAPTEST RESULT: %0d sectors verified, %0d byte mismatches, %0d failures -> %s",
		         total_secs, total_errs, total_fail,
		         (total_secs >= 10) ? "PASS" : "FAIL");
		$finish;
	end

	initial begin
		repeat (90) #1000000000;
		$display("GAPTEST: WATCHDOG timeout");
		$finish;
	end
