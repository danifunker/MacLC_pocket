...
module tb_ism_crctest;
	...
	begin
		...
		begin : parse_track
				...
				if (popbuf[k-3]==8'hA1 && popbuf[k-2]==8'hA1 && popbuf[k-1]==8'hA1 && popbuf[k]==8'hFE) begin
					...
					if (j < pops - 3 && popbuf[j]==8'hFB && (pops - (j+1)) >= 512) begin
						if (c != exp_track || h != exp_side) begin
							$display("CRCTEST: trk%0d side%0d — CHRN C=%0d H=%0d R=%0d (SEEK/SIDE WRONG)", exp_track, exp_side, c, h, r);
							...
							if ((j + 514) < pops && hsbuf[j+514][1] !== 1'b0) begin
								$display("CRCTEST: trk%0d side%0d sec%0d — DATA OK but CRC FLAG BAD (hs=%02x) ** DRIVER-VISIBLE FAIL **",
								         exp_track, exp_side, r, hsbuf[j+514]);
								total_fail = total_fail + 1;
							end
							if (errs) begin
								total_fail = total_fail + 1;
								$display("CRCTEST: trk%0d side%0d sec%0d — %0d/512 mismatches ** FAIL **", exp_track, exp_side, r, errs);
							end
							found = found + 1;
						end
					end
			...
			$display("CRCTEST: trk%0d side%0d — %0d/18 sectors captured%s", exp_track, exp_side, found,
			         (found >= 18) ? "" : "  ** INCOMPLETE **");
	...
	initial begin
		...
		$display("CRCTEST RESULT: %0d sectors verified, %0d byte mismatches, %0d failures -> %s",
	...
	initial begin
		...
		$display("CRCTEST: WATCHDOG timeout");
		$finish;
	end
