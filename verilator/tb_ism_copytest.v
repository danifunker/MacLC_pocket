...
module tb_ism_copytest;
	...
	begin
		...
		begin : parse_track
				...
				if (popbuf[k-3]==8'hA1 && popbuf[k-2]==8'hA1 && popbuf[k-1]==8'hA1 && popbuf[k]==8'hFE) begin
					...
					if (j < pops - 3 && popbuf[j]==8'hFB && (pops - (j+1)) >= 512) begin
						if (c != exp_track || h != exp_side) begin
							$display("COPYTEST: trk%0d side%0d — CHRN C=%0d H=%0d R=%0d (SEEK/SIDE WRONG)", exp_track, exp_side, c, h, r);
							...
							if (errs) begin
								total_fail = total_fail + 1;
								$display("COPYTEST: trk%0d side%0d sec%0d — %0d/512 mismatches ** FAIL **", exp_track, exp_side, r, errs);
							end
							found = found + 1;
						end
					end
			...
			$display("COPYTEST: trk%0d side%0d — %0d/18 sectors captured%s", exp_track, exp_side, found,
			         (found >= 18) ? "" : "  ** INCOMPLETE **");
	...
	initial begin
		...
		for (ti = 15; ti <= 35; ti = ti + 1) begin
			seek_to(ti);
			for (si = 0; si < 2; si = si + 1)
				read_and_verify(ti, si);
		end

		$display("COPYTEST RESULT: %0d sectors verified, %0d byte mismatches, %0d failures -> %s",
		         total_secs, total_errs, total_fail,
		         (total_fail == 0 && total_secs >= 756) ? "PASS" : "FAIL");
		$finish;
	end

	initial begin
		repeat (90) #1000000000;
		$display("COPYTEST: WATCHDOG timeout");
		$finish;
	end
