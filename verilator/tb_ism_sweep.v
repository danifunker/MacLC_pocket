...
module tb_ism_sweep;
	...
	localparam POPS = 1400;
	reg [7:0] popbuf [0:2047];
	reg [7:0] v, hs;
	integer guard, pops;
	...
	begin
		...
		// locate IDAM then DAM, compare 512 bytes at the CHRN-declared base
		found = 0; errs = 0;
		for (k = 3; k < pops - 8 && !found; k = k + 1) begin
			if (popbuf[k-3]==8'hA1 && popbuf[k-2]==8'hA1 && popbuf[k-1]==8'hA1 && popbuf[k]==8'hFE) begin
				c = popbuf[k+1]; h = popbuf[k+2]; r = popbuf[k+3]; n = popbuf[k+4];
				for (j = k+7; j < pops - 3 && !found; j = j + 1) begin
					if (popbuf[j-3]==8'hA1 && popbuf[j-2]==8'hA1 && popbuf[j-1]==8'hA1 && popbuf[j]==8'hFB) begin
						found = 1;
						if (pops - (j+1) < 512) begin
							$display("SWEEP: trk%0d side%0d — field truncated (%0d data bytes)", exp_track, exp_side, pops-(j+1));
							...
							if (c != exp_track || h != exp_side) begin
								$display("SWEEP: trk%0d side%0d — CHRN says C=%0d H=%0d (SEEK/SIDE WRONG)", exp_track, exp_side, c, h);
								total_fail = total_fail + 1;
							end else
								$display("SWEEP: trk%0d side%0d sec%0d — %0d/512 mismatches %s",
								         exp_track, exp_side, r, errs, errs ? "** FAIL **" : "ok");
							if (errs) total_fail = total_fail + 1;
						end
					end
				end
			end
		...
		if (!found) begin
			$display("SWEEP: trk%0d side%0d — NO complete field in %0d pops ** FAIL **", exp_track, exp_side, pops);
			total_fail = total_fail + 1;
		end
	end
	endtask
	...
	initial begin
		...
		tracks[0]=0; tracks[1]=1; tracks[2]=16; tracks[3]=40; tracks[4]=64; tracks[5]=79;
		...
		for (ti = 0; ti < 6; ti = ti + 1) begin
			...
			for (si = 0; si < 2; si = si + 1)
				read_and_verify(tracks[ti], si);
		end

		$display("SWEEP RESULT: %0d sectors verified, %0d byte mismatches, %0d failures -> %s",
		         total_secs, total_errs, total_fail,
		         (total_fail == 0 && total_secs >= 12) ? "PASS" : "FAIL");
		$finish;
	end

	initial begin
		repeat (30) #1000000000;
		$display("SWEEP: WATCHDOG timeout");
		$finish;
	end
